...
public class tank {
	...
	private Dir Dirs[] = {
			Dir.L, Dir.LU, Dir.U, Dir.UR, Dir.R, Dir.RD, Dir.D, Dir.DL
	};
	...
	private static final int TSize = imgs[1].getHeight(null);
	...
	public void move(){
		...
		if((this.x < 0+ TSize/2 && (dir == Dir.L || dir == Dir.DL || dir == Dir.LU))
			|| (this.y < 0+TSize/2 && (dir == Dir.U||dir == Dir.LU || dir == Dir.UR))
			|| (this.x > yardClient.xSize -TSize && (dir == Dir.R||dir == Dir.UR || dir == Dir.RD))
			|| (this.y > yardClient.ySize - TSize) &&(dir == Dir.D || dir == Dir.DL ||dir == Dir.RD)) dir = Dir.STOP;
	...
	public void superFire(){
		for(int i = 0; i < 7 ; i++){
			bullets.add(new bullet(this.x + tank.TSize/2 - bullet.BSize/2, this.y + tank.TSize/2 - bullet.BSize/2, Dirs[i]));
		}
	}
	...
	public Rectangle getRect(){
		return new Rectangle(this.x, this.y, imgs[1].getHeight(null), imgs[1].getWidth(null));
	}
